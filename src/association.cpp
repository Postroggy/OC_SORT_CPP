...
namespace ocsort {
    ...
    Eigen::MatrixXf giou_batch(const Eigen::MatrixXf &bboxes1, const Eigen::MatrixXf &bboxes2) {
        ...
        if ((wc.array() > 0).all() && (hc.array() > 0).all())
            ...
            giou = (giou.array() + 1) / 2.0;// Scale giou from (-1,1) to (0,1)
            return giou;
        }
    }
    ...
    std::tuple<std::vector<Eigen::Matrix<int, 1, 2>>, std::vector<int>, std::vector<int>> associate(Eigen::MatrixXf detections, Eigen::MatrixXf trackers, float iou_threshold, Eigen::MatrixXf velocities, Eigen::MatrixXf previous_obs_, float vdc_weight) {
        ...
        Y = std::get<0>(result);
        X = std::get<1>(result);
        Eigen::MatrixXf inertia_Y = velocities.col(0);// Shape is (2,1), need transpose for broadcasting
        ...
        // After calculating cosine of two vectors, clamp values to -1 ~ 1 for correctness
        ...
        Eigen::MatrixXf scores = detections.col(detections.cols() - 2).replicate(1, trackers.rows());// Take the second to last column, fixme: confidence level, I think.
        ...
        if (std::min(iou_matrix.cols(), iou_matrix.rows()) > 0) {
            ...
            // float and int cannot be directly compared using ==, simulating a.sum(1).max() == 1 and a.sum(0).max()
            if ((fabs(sum1 - 1) < 1e-12) && (fabs(sum0 - 1) < 1e-12)) {
                // The following operation is a simulation: np.stack(np.where(a), axis=1)
                ...
                for (int i = 0; i < cost_matrix.rows(); i++) {
                    for (int j = 0; j < cost_matrix.cols(); j++) {
                        cost_iou_matrix[i][j] = -cost_matrix(i, j);// note: negate
                    }
                }
            ...
            matched_indices = Eigen::MatrixXf(0, 2);// Otherwise, return an empty (0,2) matrix
        }
        // fixme: This is Z^{remain}_t, detections that are not matched with tracks
        std::vector<int> unmatched_detections;// This is the index of unmatched detections
        for (int i = 0; i < detections.rows(); i++) {
            if ((matched_indices.col(0).array() == i).sum() == 0) {
                unmatched_detections.push_back(i);
            }
        }
        // fixme: This is T^{remain}_t, tracks that are not matched with detections
        std::vector<int> unmatched_trackers;// Also record the index
        ...
        // Row traverse matched_indices
        Eigen::Matrix<int, 1, 2> tmp;// Actually the shape is fixed (1,2)
        ...
        if (matches.size() == 0) {
            // It should return an empty vector. After all initializations, clear it?
            matches.clear();
        }
        /**
           * Returns matches => vector<Eigen::Matrix<int,1,2>> array
           * unmatched_detections => vector<int> representing the index of unmatched detections
           * unmatched_trackers => vector<int> representing the index of unmatched tracks
       */
